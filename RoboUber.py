...
def runRoboUber(worldX, worldY, runTime, stop, junctions=None, streets=None, interpolate=False, outputValues=None,
    ...
    f = open("UberOutput.txt", "w+")
    f.write("Uber Simulation Data\n")
    f.close()
    taxisRunning = True
    while threadTime < threadRunTime and taxisRunning == True:
        ...
        if stop.is_set():
            ...
            if doPrintOutput:
                taxiIter = 1
                print(f"Dispatcher Revenue: {dispatcher0._revenue}")
                dispatcherAccounts.append(dispatcher0._revenue)
                for ta in taxis:
                    print(f"Taxi Account {taxiIter}: {ta._account}")
                    accounts[taxiIter - 1].append((float(ta._account)))
                    #accounts[taxiIter - 1][curTime] = (float(ta._account))
                    if ta._account <= 0 and taxiIter not in bankruptTaxis:

                        print(f"Taxi Account {taxiIter} Went Bankrupt At Balance: {ta._account} On Day: {curTime}")
                        bankruptTaxis.append(taxiIter)
                        f = open("UberOutput.txt", "a+")
                        f.write(f"Taxi Account {taxiIter} Went Bankrupt At Balance: {ta._account} On Day: {curTime}\n")
                        f.close()
                    if len(taxis) == len(bankruptTaxis) and taxiIter == len(taxis):
                        print("STOPPING Simulation, all taxis bankrupt")
                        dataplot.plotTaxiAccount(accounts, len(taxis), dispatcherAccounts)
                        taxisRunning = False
                        threadRunTime = 0
                    taxiIter += 1
            if threadTime != svcArea.simTime:
                threadTime += 1
            time.sleep(uberSpeedTime)
...
displaySurface = pygame.display.set_mode(size=displaySize,
                                         flags=pygame.SCALED)  # |pygame.SCALED arrgh...new in pygame 2.0, but pip install installs 1.9.6 on Ubuntu 16.04 LTS
backgroundRect = None
...
while curTime < runTime:
    ...
    # event queue had no 'q' keyboard events. Continue.
        ...
        if 'time' in outputValues and len(outputValues['time']) > 0 and curTime != outputValues['time'][-1]:
            doPrintOutputEnd = True
            if doPrintOutputEnd:
                print("curTime: {0}, world.time: {1}\n".format(curTime, outputValues['time'][-1]))

...
class Dispatcher:
    ...
    def _allocateFare(self, origin, destination, time):
        ...
        useOriginalFare = False

        fareBiddingTime = 3
        taxis = self._fareBoard[origin][destination][time].bidders

        #taxis[x][0] = Availability
        #taxis[x][1] = Taxi Rating

        #currentTaxis = numpy.ndarray(shape=(len(self._taxis),3))

        taxiRepo = {}

        if not useOriginalFare:
            if self._parent.simTime - time > fareBiddingTime:
                for taxiIdx in range(len(self._taxis)):
                    if len(self._taxis) > taxiIdx:
                        taxiRepo[taxiIdx] = ["Available", 0, taxiIdx]
                    #print(taxis)
                if len(taxiRepo) > 0:
                    for tId in taxiRepo:
                        currentLocNode = self._parent.getNode(self._taxis[tId].currentLocation[0], self._taxis[tId].currentLocation[1])
                ...
                for i in taxiRepo:
                    #print(i)
                    if taxiRepo[i][0] == "Available":
                        if taxiRepo[i][1] < lowestVal:
                            lowestVal = taxiRepo[i][1]
                            lowestId = taxiRepo[i][2]
                #print(f"Lowest: {lowestVal}")

                taxiRepo[lowestId][0] = "Unavailable"
                print(f"Choosing Lowest Value Of: {lowestId} With A Score Of: {lowestVal} It Is Now: {taxiRepo[lowestId][0]}")

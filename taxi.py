        ...
        if useOriginalPlanPath == True:
            ...
            while len(expanded) > 0:
                ...
                while len(nextTarget) > 0 and nextPos[0] in explored:
                    nextPos = nextTarget.popitem()
                if len(nextTarget) == 0:
                    del expanded[lowestCost]
                if nextPos[0] not in explored:
                    explored.add(nextPos[0])
                    expansionTargets = [node for node in self._map[nextPos[0]].items() if node[0] not in explored]
